...
export const LogModal: React.FC<LogModalProps> = ({ visible, onClose, onLogCreated }) => {
      ...
      switch (selectedType) {
        case 'mood':
          await logService.createMoodLog({
            ...
            note: moodNote,
            mood_tag_ids: selectedMoodTagIds,
          });
        ...
        case 'sleep':
          await logService.createSleepLog({
            user_id: userId,
            hours_slept: parseFloat(sleepHours),
            sleep_quality_tag_id: sleepQuality,
            notes: sleepNote,
  ...
  const renderTypeSelection = () => (
    <View style={logModalStyles.typeSelection}>
      ...
      >
        <Icon name="emoticon-happy-outline" size={32} color={selectedType === 'mood' ? '#fff' : '#007AFF'} />
      ...
      >
        <Icon name="sleep" size={32} color={selectedType === 'sleep' ? '#fff' : '#9C27B0'} />
      ...
      >
        <Icon name="pill" size={32} color={selectedType === 'medication' ? '#fff' : '#4CAF50'} />
